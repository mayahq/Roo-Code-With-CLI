...
// Create HTTP client
const createClient = (port: number, secret: string): AxiosInstance => {
...
program
	...
	.action(async (profileName: string, options: SaveCommandOptions) => {
		const port = getPort()
...
program
	...
	.action(async (modeSlug: string, profileIdOrName: string) => {
		const port = getPort()
		const secret = getSecret()
		const client = createClient(port, secret)
