...
import { EXPERIMENT_IDS, ExperimentId, experiments as Experiments } from "../shared/experiments"
import {
...
import { defaultModeSlug, getFullModeDetails, getModeBySlug } from "../shared/modes"
		...
		if (this.consecutiveMistakeCount >= this.consecutiveMistakeLimit) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Roo Code With CLI uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
