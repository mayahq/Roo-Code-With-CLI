...
export class ConfigBridgeServer {
	 ...
	 */
	private async handleRequest(req: http.IncomingMessage, res: http.ServerResponse, path: string): Promise<void> {
	 ...
	 */
	private async handleSaveConfig(res: http.ServerResponse, body: any): Promise<void> {
		try {
	 ...
	 */
	private async handleLoadConfig(res: http.ServerResponse, body: any): Promise<void> {
		try {
			if (!body.name) {
				res.writeHead(400, { "Content-Type": "application/json" })
	 ...
	 */
	private async handleListConfig(res: http.ServerResponse): Promise<void> {
		try {
	 ...
	 */
	private async handleDeleteConfig(res: http.ServerResponse, body: any): Promise<void> {
		try {
			if (!body.name) {
				res.writeHead(400, { "Content-Type": "application/json" })
				res.end(JSON.stringify({ error: "Missing required parameter: name" }))
	 ...
	 */
	private async handleSetModeConfig(res: http.ServerResponse, body: any): Promise<void> {
		try {
	 ...
	 */
	private async handleGetModeConfig(res: http.ServerResponse, query: any): Promise<void> {
		try {
			if (!query.mode) {
				res.writeHead(400, { "Content-Type": "application/json" })
